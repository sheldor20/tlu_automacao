    ...
    <div className="management-chart-wrap">
      <svg className="management-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={series.map((item) => item.label).join(" e ")}>
        ...
        {labels.map((label, index) => (
          <text key={`${label}-${index}`} x={x(index)} y={height - 14} textAnchor="middle" className="chart-axis-label">{label}</text>
        ))}
          ...
          return (
            <g key={item.label}>
              ...
              {validPoints.map((point) => (
                <circle key={`${item.label}-${point.index}`} cx={x(point.index)} cy={y(point.value)} r="4" fill="white" stroke={item.color} strokeWidth="3">
          ...
          return (
            <g key={`${label}-${labelIndex}`}>
              {series.map((item, seriesIndex) => {
                ...
                return <rect key={item.label} x={barX} y={padding.top + chartHeight - barHeight} width={Math.max(barWidth - 3, 2)} height={barHeight} rx="4" fill={item.color}><title>{`${item.label}: ${value.toLocaleString("pt-BR")}`}</title></rect>;
              })}
              <text x={center} y={height - 14} textAnchor="middle" className="chart-axis-label">{label}</text>
            </g>
          );
        })}
      </svg>
