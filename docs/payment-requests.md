...
O distrato usa os campos da aba **Solicitafção de Pagamento** da planilha fornecida: cancelamento, atraso de obra, cliente/contrato/lote/quadra/processo, responsabilidade pelo IPTU, restituição, IPTU, honorários, custas e danos morais/materiais. O valor a pagar no distrato é Restituição menos Honorários advocatícios, Custas processuais e Danos morais/materiais. O IPTU é descontado somente quando a responsabilidade é da empresa/Terra Lotus; quando é do cliente, não reduz o valor a pagar. As únicas responsabilidades disponíveis são Empresa/Terra Lotus e Cliente. A prévia e a validação no servidor usam a mesma fórmula, com cálculo em centavos. O campo Contrato é opcional no distrato. Textos de instrução, assinaturas e operações históricas de outras abas da planilha não viraram regras ou solicitações reais.
...
Fluxo: recebida → em análise → aprovada → agendada → paga → Finalizado. É possível pagar uma aprovada sem etapa de agendamento. Em etapas abertas, gestores podem pedir informações ou cancelar. Recusa é permitida durante a análise. Somente gestores podem passar de Paga para Finalizado. A finalização registra a data, preserva a data do pagamento e envia o e-mail de atualização ao solicitante. No fluxo de status, solicitações pagas permitem apenas a finalização; finalizadas não aceitam novas mudanças de status, respostas ou anexos. A gestão pode corrigir os dados ou excluir o pedido pelas ações descritas abaixo. Respostas de solicitantes ao pedido de informações devolvem a solicitação à análise.
...
## Edição e exclusão pela gestão

Na página de uma solicitação, administradores e usuários com **Gerenciar pagamentos** podem **Editar solicitação** ou **Excluir solicitação**, inclusive após encerramento. A edição abre o formulário preenchido e reaproveita as regras de cada tipo (campos opcionais de materiais, cálculo líquido do distrato e contrato opcional). Protocolo, origem, vínculo do solicitante com o usuário, datas do fluxo, status e documentos são preservados. Trocar para serviço/distrato quando já paga/finalizada exige um comprovante existente. Uma empresa mantida pode continuar no pedido mesmo que não conste na última carga do Qlik; mudanças de empresa exigem uma opção da carga atual.

A exclusão pede confirmação e é lógica: retira a solicitação das listas, bloqueia o acompanhamento, download e novas ações, mantendo documentos e histórico para auditoria. `deleted_at` e `deleted_by` identificam a exclusão. A tabela privada `payment_request_revisions` guarda quem alterou/excluiu e os dados antes/depois, sem tokens nem metadados de idempotência. Cada edição/exclusão gera um evento e um e-mail transacional; o e-mail de exclusão informa que o acompanhamento foi desativado.

`PUT /api/payments/:id` e `DELETE /api/payments/:id` exigem gestor autenticado e a versão atual da solicitação. A função `manage_payment_request` repete a autorização, bloqueia a linha e grava alteração, auditoria e notificação na mesma transação. A versão evita sobrescrever alterações simultâneas. RLS oculta registros excluídos e seus arquivos/eventos mesmo no acesso direto à API de dados. Links públicos e usuários sem permissão de gestão não podem editar/excluir.
