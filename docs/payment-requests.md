...
Os quatro formulários são: serviço (PF/PJ, escopo e data), materiais/insumos (quantidade, unidade, preço e entrega), distrato e boletos/contas. Todos coletam empresa, descrição, valor, prazo, orçamento máximo opcional, beneficiário, pagamento e propostas já recebidas. Os valores de materiais e distratos são calculados e conferidos no servidor. Um valor acima do orçamento máximo recebe um aviso para a gestão.

O distrato usa os campos da aba **Solicitafção de Pagamento** da planilha fornecida: cancelamento, atraso de obra, cliente/contrato/lote/quadra/processo, responsabilidade pelo IPTU, restituição, IPTU, honorários, custas e danos morais/materiais. Textos de instrução, assinaturas e operações históricas de outras abas da planilha não viraram regras ou solicitações reais.
